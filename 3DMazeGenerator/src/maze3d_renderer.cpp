...
Maze3DRenderer::Maze3DRenderer(sf::RenderWindow& new_window, Maze3D& maze, sf::Vector3f& cells_size)
    : window(new_window) {
  ...
  //                   static_cast<float>(window.getSize().z - 1) / maze.grid_size.z};

  maze.cell_size = cells_size;
}

void Maze3DRenderer::draw_grid(Maze3D& maze) {
  // Add 3D Rendering
  ...
  for (int y = 0; y < maze.grid_size.y; y++) {
    for (int x = 0; x < maze.grid_size.x; x++) {
      int index = maze.index_at_pos(sf::Vector3i(x, y, 0));
...
void Maze3DRenderer::draw_path(Maze3D& maze) {
  return;
...
void Maze3DRenderer::set_color(sf::Color& new_grid_color, sf::Color& new_path_color) {
