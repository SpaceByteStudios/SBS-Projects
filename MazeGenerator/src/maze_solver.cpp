...
void animate_solve_random_walk_maze(MazeRenderer& renderer, Maze& maze) {
  ...
  while (true) {
    ...
    current_cell.setPosition({maze.path.back().x * maze.cell_size.x, maze.path.back().y * maze.cell_size.y});
    ...
    for (const sf::Vector2u& next_cell : next_cells) {
      ...
      if (visited_count >= next_update_cell) {
        ...
        if (displayed_percent != last_displayed_percent) {
          ...
          std::cout << "\rSolving maze: " << std::fixed << std::setprecision(1) << displayed_percent << "% completed"
                    << std::flush;
        }
...
void animate_solve_dijkstra_maze(MazeRenderer& renderer, Maze& maze) {
  std::vector<int> dist(maze.grid_size.x * maze.grid_size.y, 1e9);
  std::vector<int> parent(maze.grid_size.x * maze.grid_size.y, -1);
  std::vector<bool> visited(maze.grid_size.x * maze.grid_size.y, false);

  dist[maze.index_at_pos(maze.start_cell)] = 0;

  std::vector<Node> nodes_vector;
  nodes_vector.push_back({0, 0, maze.start_cell});

  while (!nodes_vector.empty()) {
    // Handle window events (once per frame)
    while (const std::optional event = renderer.window.pollEvent()) {
      if (event->is<sf::Event::Closed>()) {
        renderer.window.close();
      }
    ...
    while (!nodes_vector.empty() && processed < batch_size) {
      std::sort(nodes_vector.begin(), nodes_vector.end(), [](const Node& a, const Node& b) { return a.dist < b.dist; });

      Node top_node = nodes_vector.front();
      ...
      for (sf::Vector2u& next_cell : next_cells) {
        ...
        if (alt_dist < dist[next_index]) {
          dist[next_index] = alt_dist;
          parent[next_index] = top_index;
          nodes_vector.push_back({alt_dist, alt_dist, next_cell});
        }
      }
    ...
    for (int i = 0; i < visited.size(); ++i) {
      ...
      sf::Vector2u pos = maze.pos_at_index(i);

      sf::RectangleShape maze_cell({maze.cell_size.x, maze.cell_size.y});

      sf::Vector2f maze_cell_pos =
          sf::Vector2f(pos.x * maze.cell_size.x, pos.y * maze.cell_size.y) + sf::Vector2f(1.0f, 1.0f);

      maze_cell.setPosition(maze_cell_pos);
      maze_cell.setFillColor(sf::Color(16, 16, 64)); // visited
      renderer.window.draw(maze_cell);
    }

    // Draw frontier
    for (const Node& node : nodes_vector) {
      sf::RectangleShape maze_cell({maze.cell_size.x, maze.cell_size.y});

      sf::Vector2f maze_cell_pos =
          sf::Vector2f(node.pos.x * maze.cell_size.x, node.pos.y * maze.cell_size.y) + sf::Vector2f(1.0f, 1.0f);

      maze_cell.setPosition(maze_cell_pos);
      maze_cell.setFillColor(sf::Color(32, 255, 32)); // frontier
      renderer.window.draw(maze_cell);
    }
...
void animate_solve_astar_maze(MazeRenderer& renderer, Maze& maze) {
  ...
  int est_dist_start = heuristic(maze.start_cell, maze.end_cell);
  nodes_vector.push_back({0, est_dist_start, maze.start_cell});

  const int batch_size = 2;

  while (!nodes_vector.empty()) {
    // Poll window events once per frame
    while (const std::optional event = renderer.window.pollEvent()) {
      if (event->is<sf::Event::Closed>()) {
        renderer.window.close();
      }
    ...
    while (!nodes_vector.empty() && processed < batch_size) {
      std::sort(nodes_vector.begin(), nodes_vector.end(),
                [](const Node& a, const Node& b) { return a.estimated_dist < b.estimated_dist; });
      ...
      if (closed[top_index] || top_node.dist > dist[top_index])
        continue;
      ...
      for (sf::Vector2u& next_cell : maze.get_neighbors(top_node.pos)) {
        if (!maze.is_path_free(top_node.pos, next_cell))
          continue;
        ...
        if (alt_dist < dist[next_index]) {
          dist[next_index] = alt_dist;
          parent[next_index] = top_index;
          nodes_vector.push_back({alt_dist, est_dist, next_cell});
        }
      }
    ...
    for (int i = 0; i < closed.size(); ++i) {
      ...
      sf::Vector2f maze_cell_pos =
          sf::Vector2f(pos.x * maze.cell_size.x, pos.y * maze.cell_size.y) + sf::Vector2f(1.0f, 1.0f);
      maze_cell.setPosition(maze_cell_pos);
      maze_cell.setFillColor(sf::Color(16, 16, 64)); // Blue = visited
      renderer.window.draw(maze_cell);
    }
    ...
    for (const Node& node : nodes_vector) {
      ...
      sf::Vector2f maze_cell_pos =
          sf::Vector2f(node.pos.x * maze.cell_size.x, node.pos.y * maze.cell_size.y) + sf::Vector2f(1.0f, 1.0f);
      maze_cell.setPosition(maze_cell_pos);
      maze_cell.setFillColor(sf::Color(32, 255, 32)); // Green = frontier
      renderer.window.draw(maze_cell);
    }
